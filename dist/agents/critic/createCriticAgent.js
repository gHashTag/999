...
export function createCriticAgent({ allTools, apiKey, modelName, log, }) {
    return createAgent({
        name: "Critic Agent",
        ...
        system: async ({ network }) => {
            ...
            const state = network?.get("network_state") || {};
        ...
        tools: allTools.filter((tool) => tool.name !== "createOrUpdateFiles" && tool.name !== "terminal"),
        // Removed lifecycle and onFinish hook - critique extraction will happen elsewhere if needed
    });
}
//# sourceMappingURL=createCriticAgent.js.map