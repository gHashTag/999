 ...
 */
export function extractCritiqueData(result, log) {
    if (!result)
        return { error: "Agent result is missing." };
    let outputText = "";
    // Check if output is an array of messages (standard structure)
    if (Array.isArray(result.output) && result.output.length > 0) {
        const lastMessage = result.output[result.output.length - 1]; // Assert as base Message
        ...
        if (lastMessage?.role === "assistant") {
            ...
            if (assistantMessage.type === "text") {
                ...
                if (typeof assistantMessage.content === "string") {
                    ...
                    log("warn", "extractCritiqueData", "TextMessage content was not a simple string, converted.", { content: assistantMessage.content });
                }
            }
            else if (assistantMessage.type === "tool_call") {
                log("warn", "extractCritiqueData", "Last message was tool_call, critique extraction might be incomplete.", { lastMessage: assistantMessage } // Use asserted type
                );
                ...
                log("warn", "extractCritiqueData", `Unexpected message type '${unknownType}' in last assistant message.`, { lastMessage: assistantMessage } // Use asserted type
                );
            ...
            log("warn", "extractCritiqueData", "Last message was not from assistant role.", { lastMessage });
    ...
    const likelyErrorMessage = (critiqueLower.includes("error:") || critiqueLower.includes("failed:")) &&
    ...
    if (isApproved) {
        ...
        log("warn", "extractCritiqueData", "Ambiguous critique, defaulting to needsRevision=true", { outputText });
        return {
            critique: outputText,
            needsRevision: true,
            isApproved: false,
...
//# sourceMappingURL=extractCritiqueData.js.map