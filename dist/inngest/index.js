...
async function codingAgentHandler({ event, step, }) {
    ...
    if (!step) {
        log("error", "STEP_UNDEFINED_DEBUG", "Step object is undefined inside handler!", { eventId: event?.id });
...
export const codingAgentFunction = inngest.createFunction({ id: "coding-agent-tdd-function", name: "TDD Coding Agent Function" }, // Updated ID and Name
...
const start = async () => {
    try {
        ...
        log("info", "FASTIFY_SERVER_START", `Minimal Fastify server listening on port ${PORT}`);
    }
    catch (err) {
