...
import { TddNetworkState, CodingAgentEvent } from "./types.js"
...
async function codingAgentHandler({
  ...
  try {
    ...
    const getSandboxForTools = async (id: string): Promise<Sandbox | null> => {
    ...
    const finalState: TddNetworkState | undefined =
...
if (process.env.NODE_ENV !== "test") {
  app.listen(APP_PORT, () => {
    log(
      "info",
      "SERVER_START",
      `Inngest server listening on http://localhost:${APP_PORT}/api/inngest`,
      { port: APP_PORT }
    )
  })
}
