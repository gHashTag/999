...
6.  **🧪 Продвинутое Тестирование (TDD):**
    - ❌ Написание тестов для **внутренней логики** шагов инструментов (callback-функций, переданных в `step.run`) (Отложено - сложность мокирования).
    - ❌ ~~Написание тестов для логики обработчиков инструментов.~~ (Объединено и отложено).
...
2.  ~~Перейти к Задаче 6: **🧪 Продвинутое Тестирование (TDD)**:~~ (Unit-тесты отложены)
3.  **Провести ручное тестирование:** Запустить агент с простой задачей через Inngest UI и проверить результат/логи.
4.  (Позже) Вернуться к улучшению типизации в `src/index.ts`.
5.  (Позже) Вернуться к написанию unit-тестов для handler-ов.
