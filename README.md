...
## 🧪 Паттерны Тестирования и Лучшие Практики (Уроки Стабилизации)

Стабильное и предсказуемое тестирование – ключ к успешной разработке с AI-агентами. Недавний опыт стабилизации типов выявил следующие важные практики:

1.  **🕉️ Чистота Типов Прежде Всего:**

    - **Правило:** _Никогда_ не запускать тесты (`bun test`) до тех пор, пока проверка типов (`bun exec tsc --noEmit`) не пройдет успешно **без единой ошибки**.
    - **Цикл:** После _любых_ изменений кода (особенно в общих файлах вроде `testSetup.ts`) немедленно запускать `tsc`. Исправлять ошибки типов -> повторять проверку -> только потом переходить к тестам.

2.  **🧱 Изолированный и Стабильный `testSetup.ts`:**

    - **Фундамент:** Файл `src/__tests__/setup/testSetup.ts` является основой тестового окружения. Он должен быть максимально чистым, понятным и **типобезопасным сам по себе**.
    - **Экспорт/Импорт Типов:** Избегать сложных схем ре-экспорта типов, если они приводят к конфликтам. В нашем случае сработал паттерн **индивидуального ре-экспорта** (`export type AgentDependencies = AgentDependenciesType; export type BaseLogger = ...`) в `testSetup.ts`. Тестовые файлы должны импортировать типы из `testSetup.ts`.
    - **Моки:** Все общие моки (логгер, KV, базовые инструменты) должны быть определены и экспортированы здесь.

3.  **♟️ Стратегия Мокирования:**

    - **Простые Заглушки:** При отладке сложных проблем или настройке тестов для новых компонентов начинать с **простых заглушек** для внешних зависимостей (например, `mockSandbox: any = undefined`, `createMockAgent: any = undefined`). Это помогает изолировать ошибки.
    - **Постепенное Уточнение:** После стабилизации основной части, постепенно раскомментировать/усложнять моки и исправлять ошибки, которые они вызывают в тестах.
    - **Конкретные Зависимости:** В тестах импортировать **только необходимые** моки/хелперы из `testSetup.ts`.
    - **Inngest Функции:** Для тестирования Inngest функций (например, хендлеров агентов) использовать `@inngest/test` и метод `t.execute()` с опцией `steps` для мокирования шагов (`step.run`, `step.invoke` и т.д.), а не всего модуля.

4.  **📉 Инкрементальный Подход к Отладке (Особенно после Слияния):**

    - При возникновении большого числа ошибок (например, после `git merge`): **не паниковать**. Сосредоточиться на **одной** ключевой области (например, `testSetup.ts` или конкретный модуль).
    - Использовать `tsc --noEmit` для выявления ошибок.
    - Исправлять ошибки **постепенно**, часто перепроверяя типы.
    - **Изолировать:** Если возможно, запускать проверку типов (`tsc`) или тесты (`bun test --isolate`) только для изменяемого файла/модуля.
    - **Коммитить Часто:** После достижения промежуточного стабильного состояния делать коммит.

5.  **🤖 Внимание к Инструментам Редактирования:**
    - Автоматические инструменты (`edit_file`) могут быть ненадежны при работе со сложными файлами или при внесении множественных изменений. Всегда **проверять результат** их работы.
    - При повторяющихся неудачах инструмента не бояться прибегать к **ручной замене** содержимого файла (предоставленного AI).
